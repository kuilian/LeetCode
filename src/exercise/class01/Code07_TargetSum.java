...
public class Code07_TargetSum {
    ...
    public int MyfindTargetSumWays(int[] nums, int target) {

        if (nums.length == 0 || nums == null) {
            return 0;
        }

        return myProcess(nums, 0, target, new HashMap<Integer, HashMap<Integer, Integer>>());
    }

    //可以自由使用arr[index..]中的所有位置
    //得到rest的数 返回多少种方法数
    public static int myProcess(int[] arr, int index, int rest, HashMap<Integer, HashMap<Integer, Integer>> map) {
        if (map.containsKey(index) && map.get(index).containsKey(rest)) {
            return map.get(index).get(rest);
        }
        int ans = 0;
        if (index == arr.length) {
            //只有剩余为0 才能说明搞定了 否则没搞定
            ans = rest == 0 ? 1 : 0;
        } else {
            ans = myProcess(arr, index + 1, rest - arr[index], map) +
                    myProcess(arr, index + 1, rest + arr[index], map);
    ...
    public int MyfindTargetSumWaysdp(int[] nums, int target) {

        int sum = 0;
        for (int n : nums) {
            sum += n;
        }
        return sum < target || ((target & 1) ^ (sum & 1)) != 0 ? 0 : myDP(nums, (target + sum) >> 1);
    }

    private int myDP(int[] nums, int target) {
        if (target < 0) {
            //目标为0
            return 0;
        }
        ...
        for (int i = 1; i <= N; i++) {
            for (int j = 0; j <= target; j++) {
                dp[i][j] = dp[i - 1][j];
                //要凑到当前数 当前目标数减去上一个数还是大于0才有可能能使用
                //如果要使用 则要
                if (j - nums[i - 1] >= 0) {
                    dp[i][j] += dp[i - 1][j - nums[i - 1]];
                }
